...
foreach (["8px", "20px", "128px", "256px"] as $validFontSize) {
    $fontSizeDocument = $document;
    $fontSizeDocument["blocks"]["text-1"]["props"]["content"]["content"][0]["content"][0]["marks"][0]["attrs"]["fontSize"]
        = $validFontSize;
    CmsContentValidator::validate($fontSizeDocument, 7, 12);
}

foreach (["7px", "257px", "16em", "16.5px", "016px"] as $invalidFontSize) {
    expectDomainException(static function () use ($document, $invalidFontSize): void {
        $fontSizeDocument = $document;
        $fontSizeDocument["blocks"]["text-1"]["props"]["content"]["content"][0]["content"][0]["marks"][0]["attrs"]["fontSize"]
            = $invalidFontSize;
        CmsContentValidator::validate($fontSizeDocument, 7, 12);
    }, "font size {$invalidFontSize} is rejected");
}
...
expectCmsTest(
    PageMetadataValidator::title("Une cite ancienne") === "Une cite ancienne",
    "plain page title is accepted",
);
expectDomainException(
    static fn () => PageMetadataValidator::title("https://example.com"),
    "URL in a page title is rejected",
);
expectCmsTest(
    PageMetadataValidator::filterIds([3, 1, 3]) === [3, 1],
    "page filters are deduplicated",
);
expectDomainException(
    static fn () => PageMetadataValidator::filterIds(range(1, 16)),
    "more than 15 page filters are rejected",
);
